...
function CategorySubNav({ activeCategory }: { activeCategory: string }) {
  return (
...
function KategoriDropdown({ activeCategory }: { activeCategory: string }) {
...
function HeaderSearchParamsConsumer({ variant }: { variant: "default" | "compact" }) {
  ...
  return (
    <>
        ...
        <div className="flex flex-1 items-center gap-6">
          ...
          {variant === "default" && (
            <div className="hidden max-w-3xl flex-1 items-center gap-4 md:flex">
              <KategoriDropdown activeCategory={activeCategory} />
      ...
      {/* Category sub-nav */}
      {variant === "default" && <CategorySubNav activeCategory={activeCategory} />}
...
export function SiteHeader({ variant = "default" }: SiteHeaderProps) {
  return (
    <header className="sticky top-0 z-50 flex flex-col bg-card shadow-sm">
      ...
      >
        <HeaderSearchParamsConsumer variant={variant} />
      </Suspense>
    </header>
  );
}
